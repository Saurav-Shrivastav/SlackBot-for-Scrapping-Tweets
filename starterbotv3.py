...
def handle_command(command, channel):
    ...
    if command.startswith(EXAMPLE_COMMAND):
        ...
        if len(arr) == 1:
            arr = command.split('=')
            return fetchTweets(int(arr[1]), channel)
        else:
            return fetchTweets(int(arr[2]), channel)
    else:
...
def fetchTweets(num, channel):
    ...
    csv_file = open('tweets_scrape.csv', 'w')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['Tweet', 'Link', 'Date and time'])
    ...
    for tweet in soup.findAll('li', class_ = 'js-stream-item stream-item stream-item'):
        if count > num:
            ...
            csv_writer.writerow([tweet_text, tweet_link, tweet_date])
        count = count + 1
    csv_file.close()
    my_file = {
      'file' : ('tweets_scrape.csv', open('tweets_scrape.csv', 'rb'), 'csv')
      }

    payload={
      "filename":"tweets_scrape.csv",
      "token":os.environ.get('SLACK_BOT_TOKEN'),
      "channels":[channel],
    }

    r = requests.post("https://slack.com/api/files.upload", params=payload, files=my_file)
    print(r)  #prints the response of the file upload
